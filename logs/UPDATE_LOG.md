...
### 功能增强
- ✅ **桌面端登陆注册按钮优化**：提升登录注册按钮的视觉效果和交互体验
- ✅ **站外邮箱通知设计优化**：改进邮件通知的设计，提升用户体验
- ✅ **首屏加载逻辑优化**：优化首页加载速度，提升用户体验
- ✅ **侧边栏优化**：改进侧边栏的视觉效果和交互体验
- ✅ **修改密码功能实现**：新增修改密码功能，允许用户修改密码
- ✅ **忘记密码功能实现**：新增忘记密码功能，支持密码重置
- ✅ **协议日期更新**：更新用户协议和隐私政策的日期
- ✅ **移动端侧边栏触发器优化**：改进移动端侧边栏的触发方式
- ✅ **站内通知功能完善**：增强站内通知功能，提升用户体验
- ✅ **用户资料排版调整**：优化用户资料页面的排版布局
- ✅ **首页帖子展示逻辑调整**：改进首页帖子的展示逻辑
- ✅ **音频样式简化**：简化音频播放器的样式，提升视觉效果

### Bug修复
- ✅ **未登录状态下评论区登录按钮跳转问题**：修复未登录状态下点击评论区的登录按钮不能跳转的错误
- ✅ **音频时间夜间模式显示问题**：修复音频时间显示在夜间模式下看不清的问题
- ✅ **桌面端未登录状态登录注册入口问题**：修复桌面端退出登录后(或未登录状态)，没有登陆注册入口的问题
- ✅ **用户资料下帖子删除按钮重叠问题**：调整由于用户资料下的帖子边距而产生的帖子删除按钮与帖子内容重叠的问题
- ✅ **用户资料下帖子边距调整**：优化用户资料页面下帖子的边距

---

## 历史更新：移动端侧边栏、音频支持与文件上传优化
...
### 功能增强
- ✅ **移动端侧边栏优化**：实现固定左侧触发、从左侧滑入居中显示，使用 React Portal 渲染到 document.body
- ✅ **音频支持增强**：支持在首页和详情页直接播放音频，自动从帖子内容中检测音频文件
- ✅ **文件上传优化**：支持所有文件类型，30MB 大小限制，每个帖子最多 1 个文件，仅在发布时上传
- ✅ **邮件服务限流**：实现 2 次/秒的请求限流，使用节流和批处理优化邮件发送
- ✅ **标题和内容验证调整**：标题至少 1 个字符，内容可选，提升用户体验

### Bug修复
- ✅ **帖子卡片左右边距不一致**：修复帖子卡片左右边距不对称问题，确保日期完整显示
- ✅ **音频文件名显示扩展名**：优化音频播放器，只显示文件名（不包含扩展名）
- ✅ **用户资料页面帖子卡片样式冲突**：修复选择器特异性问题，确保布局正常

### 修复内容
- ✅ **布局优化**：更新 PostCard.css、PostDetail.css 和 UserProfile.css，确保一致的边距和布局
- ✅ **音频检测增强**：搜索整个帖子内容寻找音频文件，而不仅仅是前3行
- ✅ **文件名处理优化**：使用正则表达式提取纯净文件名，移除扩展名
- ✅ **文件上传逻辑重构**：移除自动上传，仅在发布帖子时上传文件
- ✅ **表单验证调整**：修改 CreatePost.jsx，调整标题和内容的验证规则

### 修改文件
- `frontend/src/components/PostCard.jsx` - 增强音频检测，优化文件名显示
- `frontend/src/components/PostCard.css` - 修复帖子卡片左右边距
- `frontend/src/pages/PostDetail.jsx` - 添加音频支持，优化文件名显示
- `frontend/src/pages/PostDetail.css` - 修复布局间距
- `frontend/src/components/ImageUpload.jsx` - 优化文件上传，支持所有类型和30MB限制
- `frontend/src/pages/CreatePost.jsx` - 调整表单验证，修改文件上传逻辑
- `frontend/src/pages/UserProfile.css` - 修复选择器特异性问题
- `frontend/src/mockData.js` - 添加测试音频帖子
- `backend/services/emailService.js` - 实现邮件限流和批处理
...
- 2026-01-04: 添加 v1.11.0 的更新记录（优化桌面端登陆注册按钮、优化站外邮箱通知的设计、优化首屏的加载逻辑、优化侧边栏、修复未登录状态下点击评论区的登录按钮不能跳转的错误、修复音频时间显示在夜间模式下看不清的问题、修改密码功能实现、忘记密码功能实现、更新协议日期、修复桌面端退出登录后(或未登录状态)，没有登陆注册入口的问题、优化移动端侧边栏的触发器、站内通知的功能完善、调整用户资料的排版问题、调整由于(用户资料下的帖子边距)而产生的帖子删除按钮与帖子内容重叠的问题、调整移动端用户资料下的帖子边距、调整首页帖子展示的逻辑、简化音频样式）
