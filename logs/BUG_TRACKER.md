...
**问题描述**:
- 用户资料页面下，帖子删除按钮与帖子内容重叠
- 影响用户对帖子内容的查看和删除按钮的点击
...
**测试验证**:
- ✅ 验证用户资料下帖子删除按钮与帖子内容不重叠

---

### Bug #046: 桌面端退出登录后(或未登录状态)，没有登陆注册入口
...
**问题描述**:
- 桌面端退出登录后或在未登录状态下，页面上没有明显的登录注册入口
- 用户无法找到登录注册的入口，影响用户体验

**复现步骤**:
1. 登录论坛
2. 点击退出登录
3. 观察页面，发现没有明显的登录注册入口
4. 或直接在未登录状态下访问论坛，发现没有明显的登录注册入口

**根本原因**:
- 桌面端登录注册按钮的显示逻辑存在问题
- 退出登录后或未登录状态下，登录注册按钮没有正确显示
...
**测试验证**:
- ✅ 验证退出登录后，页面上有明显的登录注册入口
- ✅ 验证未登录状态下访问论坛，页面上有明显的登录注册入口

---

### Bug #045: 音频时间显示在夜间模式下看不清
...
**问题描述**:
- 未登录状态下点击评论区的登录按钮不能跳转
- 用户无法通过评论区的登录按钮快速进入登录页面
...
**问题描述**:
- 帖子卡片右侧边距没有完全占据空间，导致布局不对称
- 帖子发布日期容器宽度不足，导致日期显示不完整
- 影响整体视觉美观和用户体验
...
**修复方案**:
- 更新 `PostCard.css`，确保帖子卡片左右边距一致
- 为 `post-publish-date-container` 添加 `width: 100%`，确保完整显示日期
- 移除导致布局偏移的负外边距
- 修复 `UserProfile.css` 中的选择器特异性问题
...
**问题描述**:
- 音频播放器中显示完整文件名（包含扩展名），影响视觉体验
- 例如："audio.mp3" 显示为 "audio.mp3"，而不是期望的 "audio"
...
**根本原因**:
- `getFilenameFromUrl` 函数没有移除文件名的扩展名
- 缺少正则表达式处理来提取纯净文件名

**修复方案**:
- 修改 `getFilenameFromUrl` 函数，使用正则表达式 `/\.[^/.]+$/` 移除扩展名
- 更新 `PostCard.jsx` 和 `PostDetail.jsx` 中的文件名处理逻辑
...
- **中等**: 22 个 (#003, #007, #008, #009, #012, #013, #014, #017, #021, #023, #025, #026, #027, #028, #032, #033, #034, #035, #036, #039)
- **低**: 15 个 (#001, #004, #005, #006, #015, #016, #019, #022, #024, #029, #030, #037, #038, #040, #041)
...
## Bug #041 Skeleton Loader 缺失样式导致不显示

**状态**: 已修复  
**级别**: 低  
**版本**: v1.9.5  
**影响范围**: 所有用户（列表/搜索加载时的视觉反馈）  
**问题描述**:
- Home/Search 等页面加载中时未显示骨架屏，占位样式缺失
...
**根因**:
- 缺少 skeleton 公用样式定义，组件结构有骨架但无视觉呈现

**修复方案**:
- 补充 skeleton 渐变与尺寸样式（sm/md/lg/fade），加入 shimmer 动画
- 确保 Home/Search 的 skeleton 结构使用统一样式
...
**影响范围**: 移动端用户  
**问题描述**:
- UserProfile 卡片在小屏幕左右贴边，视觉拥挤

**复现步骤**:
1. 移动端打开用户资料页
2. 观察个人资料卡片与页面边缘几乎无留白

**根因**:
- 移动端断点缺少左右 padding 处理

**修复方案**:
- 在 768px 断点增加 `.user-profile`、`.profile-header`、`.profile-content` 横向内边距
...
## Bug #039 每日任务多端不同步导致经验值异常/重复领取

**状态**: 已修复  
**级别**: 中等  
**版本**: v1.9.5  
**影响范围**: 所有用户（多端登录、任务与经验值）  
**问题描述**:
- 多端登录后每日任务完成状态不同步，可重复领取奖励
- 经验值在临界点不升级或显示为 0 / 不更新

**复现步骤**:
1. A 端完成任务后切到 B 端查看，任务仍显示未完成
2. 重新完成任务导致经验值重复叠加或显示异常

**根因**:
- 仅前端 localStorage 存储任务状态，缺少服务端持久化与并发保护
- 经验值来源不统一，缺少服务器写入与回传

**修复方案**:
- 后端新增 `daily_tasks` 表与接口：获取/完成每日任务，写入用户经验，防重复领取
- 前端 DailyTasks 组件改为调用后端，测试用户本地兜底；新增签到任务；所有任务奖励统一 +30
- 同步经验到用户对象与本地缓存，保证进度条与等级即时更新
...
- 2025-12-08: 添加 v1.9.4 的 Bug 修复记录（#036, #037, #038）
  ...
  - Bug #037: 响应式设备上用户名、称号和编辑按钮间距冲突
  - Bug #038: VITE_ENABLE_TEST_LOGIN 环境变量逻辑不正确
- 2025-12-09: 添加 v1.9.5 的 Bug 修复记录（#039, #040, #041）
  - Bug #039: 每日任务多端不同步，经验值/升级异常，可能重复领取奖励
  - Bug #040: 移动端个人资料卡片左右贴边，视觉压迫
  - Bug #041: 部分页面的 Skeleton Loader 不显示（缺少样式定义）
- 2026-01-02: 添加 v1.10.0 的 Bug 修复记录（#042-#048）
- 2026-01-04: 添加 v1.11.0 的 Bug 修复记录
